	...
	for(int v: adj[u])
		if(v != pr)
		{
			if(disc[v] == -1)
			{
				...
				if((pr == -1 && child_cnt > 1) || (pr != -1 && disc[u] <= low[v]))
				{
					...
					st.pop_back();
					if(curr.size() == 1) bridges.push_back(curr.back());
					else bcc.push_back(curr);
				}
			}
...
void compute_bcc()
{
	...
	for(int i = 1; i <= n; i++)
		if(disc[i] == -1)
		{
			...
			if(!st.empty())
			{
				if(st.size() == 1) bridges.push_back(st.back());
				else bcc.push_back(st);
				st.clear();
			}
		}	
}
